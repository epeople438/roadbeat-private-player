...
- 底栏预留区在移动端只取 `env(safe-area-inset-bottom)` 本身，不再额外叠加 `max(..., 10px)`；`--nav-height` 在移动端为 `58px`（桌面仍为 `78px`）。这样按钮行贴到 Home 指示条上沿，不会在底部留出多余空白。改动这两个变量会同时影响迷你播放器位置和各视图的底部内边距，它们都从同一组变量推导。
